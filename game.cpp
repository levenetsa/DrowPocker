...
double couple(card* cards)
{
    ...
    if (maxx==-1) return 0;
    std::sort(wet_new,wet_new+3);
    return_am+=maxx*mass[8];
    return_am+=wet_new[2]*mass[7];
    return_am+=wet_new[1]*mass[6];
    return_am+=wet_new[0]*mass[5];
    return return_am;
}
...
double double_couple(card* cards)
{
    ...
    for(int i=0; i<3; i++)
        for(int j=i+1; j<3; j++)
            if (wet_new[i]==wet_new[j]&&maxx<wet_new[j])
            {
                maxx=wet_new[j];
            }
    if (maxx==-1) return 0;
    ...
    for(int i=0; i<3; i++)
    {
        if(maxx!=wet_new[i]&&maxx2!=wet_new[i]) return_am+=wet_new[i]*mass[9];
    }
    return return_am;
}
...
double triple(card* cards)
{
    ...
    if (maxx==-1) return 0;
    if (wet_new[0]>wet_new[1]) std::swap(wet_new[0],wet_new[1]);
    std::sort(wet_new,wet_new+3);
    return_am+=maxx*mass[14];
    return_am+=wet_new[1]*mass[13];
    return_am+=wet_new[0]*mass[12];
    return return_am;
}
...
double full_house(card* cards)
{
    ...
    if (maxx==-1) return 0;
    if (wet_new[0]!=wet_new[1]) return 0;
    return_am+=wet_new[0]*mass[21];
...
void game::round(bool g_flag)
{
    ...
    if (g_flag) folded=new bool[player_amount];
    bool getting_card[player_amount];
    for (int i=0; i<player_amount ; i++ )
    {
